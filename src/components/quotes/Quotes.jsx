...
const Quotes = () => {
  ...
  useEffect(() => {
    fetch('https://raw.githubusercontent.com/Mahfuz2411/jsonplaceholder/main/quotes.json')
    .then(res => res.json())
          ...
          return <SwiperSlide key={res.id}>
            ...
            >
              <div className="text-center flex flex-col justify-center items-center gap-4 relative z-10 text-white ">
                <p className="text-xl md:text-3xl lg:text-5xl">&quot;{res.quote}&quot;</p>
                <h2 className="text-xl md:text-3xl lg:text-5xl font-bold"> ~ {res.author}
                  <div className={`badge badge-accent badge-outline`}>{res.category}</div>
                </h2>
              </div>
            </div>
          </SwiperSlide>
